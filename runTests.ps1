...
foreach ($project in $projects) {
    $properties = dotnet msbuild $project -getProperty:TargetFrameworks -getProperty:TargetFramework -p:Configuration=Release | ConvertFrom-Json
    $frameworks = ($properties.Properties.TargetFrameworks + $properties.Properties.TargetFramework).Split(';')
    if ($Framework) {
